...
def get_data():
    ...
    ratings_path = os.path.join(current_dir, 'ml-latest-small', 'ratings.csv')

    df_movielens = pd.read_csv(ratings_path)
    return df_movielens[['userId', 'movieId', 'rating']]


if __name__ == '__main__':
    df_movielens = get_data()

    df_pred, J_hist = fill_missed(df_movielens, compute_average_for_blank_columns=False, n_features=20, max_iter=5000, return_J_hist=True)

    df_pred.to_csv('predicted_results_movielens.csv')
